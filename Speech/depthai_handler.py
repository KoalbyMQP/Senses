...
class DepthAIHandler:
    ...
    def process_frame(self, frame, detections, current_state):
        ...
        print(f"Current state: {current_state}, Target label: {target_label}")
        
        if not target_label:
            return frame
        
        # Set the target in NNetManager
        if hasattr(self, '_nnManager'):
            print(f"Setting target in NNetManager: {target_label}")
            self._nnManager.set_target_object(target_label)
            print("Calling NNetManager draw")
            self._nnManager.draw(frame, detections)
        else:
            print("No NNetManager available!")
