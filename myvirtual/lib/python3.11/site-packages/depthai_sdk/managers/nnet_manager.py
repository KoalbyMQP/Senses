...
class NNetManager:
    ...
    def __init__(self, inputSize, nnFamily=None, labels=[], confidence=0.5, sync=False, device=None):
        ...
        self.max_buffer_size = 10000  # Reduced from 500 to prevent memory issues
    ...
    def set_target_object(self, target):
        ...
        try:
            ...
            print(f"Debug: Current memory usage: {psutil.Process().memory_info().rss / 1024 / 1024:.2f} MB")
            
            if hasattr(self, 'device'):
                print(f"Debug: Device state: {self.device.getBootloader() if self.device else 'No device'}")
            else:
                print("Debug: No device attribute found")
            
            if not hasattr(self, '_target_object'):
                print("Debug: Initializing _target_object attribute")
            ...
            print(f"Debug: Final memory usage: {psutil.Process().memory_info().rss / 1024 / 1024:.2f} MB")
            
        except Exception as e:
            print(f"Error in set_target_object: {e}")
            import traceback
            traceback.print_exc()
            raise RuntimeError(f"Failed to set target object: {str(e)}")
