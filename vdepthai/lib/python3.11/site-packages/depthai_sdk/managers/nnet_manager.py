    ...
    def draw(self, source, decodedData):
        if self._outputFormat == "detection":
            def drawDetection(frame, detection):
                # Check if we should draw this detection
                if hasattr(self, '_should_draw_detection') and not self._should_draw_detection(detection):
                    return

                # Handle target object filtering if enabled
                if hasattr(self, '_target_object'):
                    ...
                    if target_detections:
                        ...
                        if detection != max_conf_detection:
                            return  # Skip if not the highest confidence detection
