...
class NNetManager:
    ...
    def draw(self, source, decodedData):
        if not decodedData: return

        def drawDetection(frame, detection):
            # Check if we should draw this detection based on target filtering
            if hasattr(self, '_target_object') and self._target_object:
                label_text = self.getLabelText(detection.label).lower()
                print(f"NNetManager - Checking detection: {label_text} against target: {self._target_object}")  # Debug print
                if label_text != self._target_object:
                    return
            
            # If we have multiple instances of target object, only draw highest confidence
            if hasattr(self, '_target_object') and self._target_object:
                target_detections = [d for d in decodedData 
                                   if self.getLabelText(d.label).lower() == self._target_object]
                if target_detections:
                    max_conf_detection = max(target_detections, key=lambda d: d.confidence)
                    if detection != max_conf_detection:
                        return
            ...
            if hasattr(detection, 'spatialCoordinates'):
                ...
                if hasattr(detection, 'boundingBoxMapping'):
                    # Use fixed 1920x1080 resolution for physical dimension calculations
                    frame_width = 1920
                    frame_height = 1080
                    ...
                    # Calculate physical dimensions using focal lengths
                    fx = 1504.128173828125
                    fy = 1504.2720947265625
