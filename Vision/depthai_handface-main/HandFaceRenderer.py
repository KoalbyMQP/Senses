        ...
        if face.lm_score > self.tracker.flm_score_thresh:
            ...
            if self.show_face_landmarks:
                ...
                if self.face_style == 0:
                    ...
                    #added for forehead
                    left_eyebrow = [face.landmarks[i, :2] for i in [70, 63, 105, 66, 107]]
                    right_eyebrow = [face.landmarks[i, :2] for i in [336, 296, 334, 293, 300]] 

                    forehead_offset = -20
                    left_forehead = [(x, y + forehead_offset) for (x, y) in left_eyebrow] 
                    right_forehead = [(x, y + forehead_offset) for (x, y) in right_eyebrow] 

                    forehead_points = np.array(left_forehead + right_forehead[::-1], dtype=np.int32)

                    cv2.polylines(self.frame, [forehead_points], isClosed=True, color=(0, 255, 0), thickness=2) 
                    cv2.fillPoly(self.frame, [forehead_points], color=(0, 255, 0, 50))
